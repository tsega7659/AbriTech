...
async function migrate() {
    ...
    try {
        ...
        const migrationSql = fs.readFileSync('migration_dual_submission.sql', 'utf8');
        await pool.query(migrationSql);
        console.log('Migration successful!');

        // Verify the table
        const [columns] = await pool.execute('SHOW COLUMNS FROM assignmentsubmission');
        console.log('\n--- assignmentsubmission Columns ---');
